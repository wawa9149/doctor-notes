...
class LLMService(LLMRepository):
    ...
    def __init__(self):
        ...
        if not self.use_llm:
            logger.warning("LLM client not initialized. Using mock responses.")
    ...
    async def generate_soap_note(
        ...
        try:
            ...
            logger.error(f"Failed to generate SOAP note with Azure OpenAI: {e}")
            return self._generate_mock_soap_note()
    ...
    async def generate_chat_response(
        ...
        try:
            ...
            logger.error(f"Failed to generate chat response with Azure OpenAI: {e}")
            return self._generate_mock_chat_response(question)
...
{summary_text}"""
    ...
    def _parse_soap_response(self, content: Optional[str]) -> SOAPNote:
        """SOAP 응답 파싱"""
        if not content:
            return self._generate_mock_soap_note("Empty response from LLM")
        try:
            ...
            logger.error(f"Failed to parse SOAP JSON response: {content}")
            return self._generate_mock_soap_note(f"Invalid JSON: {content}")
    
    def _parse_chat_response(self, content: Optional[str]) -> dict:
        """채팅 응답 파싱"""
        if not content:
            return self._generate_mock_chat_response(question="N/A", reason="Empty response from LLM")
        try:
            return json.loads(content)
        except json.JSONDecodeError:
            logger.error(f"Failed to parse Chat JSON response: {content}")
            return {
                "answer": content,
                "citations": [],
                "rolling_summary_next": "Could not parse the conversation summary."
            }
    
    def _generate_mock_soap_note(self, reason: str = "LLM not configured") -> SOAPNote:
        """모의 SOAP 노트 생성"""
        logger.info(f"Generating mock SOAP note. Reason: {reason}")
        return SOAPNote(
            subjective="환자는 두통과 어지럼증을 호소함 (Mock)",
            objective="혈압 120/80, 맥박 72회/분, 체온 36.5도. (Mock)",
            assessment="긴장성 두통 의심 (Mock)",
            plan="진통제 처방, 충분한 휴식 권고 (Mock)",
            citations=["Mock Citation"]
        )
    
    def _generate_mock_chat_response(self, question: str, reason: str = "LLM not configured") -> dict:
        """모의 채팅 응답 생성"""
        logger.info(f"Generating mock chat response. Reason: {reason}")
        return {
            "answer": f"'{question}'에 대한 모의 답변입니다. 관련 정보를 찾을 수 없습니다.",
            "citations": [],
            "rolling_summary_next": "의사와 AI가 환자 정보를 검토 중 (Mock)"
        }